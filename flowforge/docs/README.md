...
Thirty-two documents is too many to read in order, so this is a map rather than
...
| [EXPLAIN.md](./EXPLAIN.md) | *Why didn't it send the email?* — the question every workflow tool is asked and none answers. The runtime counterpart to the effect report: same control dependence, read forwards from what one run actually did. |
...
| [IMPACT.md](./IMPACT.md) | Four checks look at a candidate before it ships and none says what the edit does to the properties somebody was relying on. Every analysis over both graphs, and the difference in their verdicts — where a one-line diff can remove an approval from a payment path. |
...
| [SCHEDULE-LOAD.md](./SCHEDULE-LOAD.md) | Every timing analysis here is about one run or one workflow. Nothing modelled the machine they share — and its load is not random, because cron is written by people and people write round numbers. |
