...
| `flowforge impact <id> <file> [--strict]` | **What does this change mean?** `diff` says the file drifted, `lint` says it is valid, `contract` says whether it breaks its callers, `preview` says what last week's traffic would have done. None says what the edit does to the properties somebody was relying on — and the change that matters most is structurally tiny: delete one edge, wire a trigger at the node behind it, and an approval is no longer in front of a payment. It passes every other check in this list. So six analyses run over **both** graphs and the difference in their verdicts is the report. A property already broken is not a finding of this change; `resolved` is reported too, because a reviewer told only about the bad half cannot tell a refactor from a regression. Exits non-zero on its own for a finding another gate would refuse; `--strict` stops for the legal-but-enormous ones too ([docs](../docs/IMPACT.md)) |
...
| `flowforge schedule --workspace [ws-id] [--capacity N]` | **What a week of every schedule does to the machine they share.** The other view here is about one workflow; this is about the load, and the load is not random — cron is written by people and people write round numbers, so a workspace's scheduled work arrives in a spike nobody designed at a time nobody is awake to see. Each cron is expanded **in its own time zone** (two "midnights" in different zones do not collide), each occurrence occupies `[start, start + mean duration)` so the 40-minute job started at midnight still counts against the 00:30 one, and a sweep line finds the max overlap. Reports what share of runs land on the hour — the difference between a peak that is an accident and one that is real — plus the single minute-shift that flattens it most. A workflow that has never run is excluded and named, so the peak is honestly a floor. `--capacity N` exits non-zero over budget ([docs](../docs/SCHEDULE-LOAD.md)) |
| `flowforge backfill <id> --from <iso> --to <iso> [--yes]` | Re-run a scheduled workflow over a window of the past — the tail of a recovery script: *deploy the fix, then replay the window it was broken for*. **Previews by default** and refuses to submit without `--yes`, because it is the one verb here that turns a date range into real runs against real systems |
| `flowforge audit <workspace-id> [--limit N] [--action secret.*] [--verify]` | The workspace's **tamper-evident audit log**, and with `--verify` an integrity check of the hash chain. The exit code is the point of that form: an integrity check nobody runs is not a control, so it exits non-zero on a broken chain — which is all a cron line needs to page somebody |
...
| `flowforge explain <exec-id> [--node <id>]` | **Why did this run do what it did?** `run` will tell you the email step says `skipped` — the fact somebody already has when they come asking. This says *why*: the decision that closed the path to it, the outcome that decision took, and for an FXL condition the values it read (`total > 100 — total was 850`), read out of the recorded input rather than re-derived. It names the **deepest** gate that closed the path, because everything upstream of that is why *that* gate was reached. `--node` narrows to the question actually asked: nobody wants a run explained, they want to know why *that* did not happen ([docs](../docs/EXPLAIN.md)) |
...
| `flowforge effects <id> [--deep] [--ungated]` | **What a run can do to the outside world, and what has to be true first** — every HTTP call, email, Slack post, sub-workflow and model call, each with the decisions it is control-dependent on. The question a promotion review opens with, and the one no other command here answers. An effect with no conditions runs on *every* run — including one whose gate a second trigger routes around; `--ungated` fails the build on those. **`--deep` follows sub-workflow calls** — one box on the canvas is an entire other workflow at run time, and the preconditions it reports are the conjunction of the caller's gate on the call and the callee's gate on the effect ([docs](../docs/EFFECTS.md)) |
| `flowforge exposure [ws-id] [--unchecked] [--top N]` | **Which workflow should I look at first?** The only command here that does not take a workflow id, because it is the only one that answers the question you have before you have one. Ranks a whole workspace by *outward actions per day* — what a run can reach, including the effects several sub-workflow calls away, multiplied by how often it actually runs. The figure is an **interval**: a floor of effects nothing gates and a ceiling of all of them, ranked by the ceiling because a gate nobody has tested is not a gate. Runs a caller started are charged to the caller, so a shared utility is not ranked above the workflow that decides to invoke it. Scenarios, guarantees, assertions and drift monitoring are counted **beside** the number and never folded into it — four scenarios do not make a workflow four units safer. `--unchecked` fails the build when anything has consequence and no checks at all ([docs](../docs/EXPOSURE.md)) |
| `flowforge repeats <id> [--strict]` | **What happens twice?** Every other check here asks whether one run is right. Three mechanisms make a step run a second time — node retries (three attempts by default, on *every* run), resume-from-failure, and crash recovery — and nothing told an author what their graph does under any of them. Each step gets a verdict: `safe` (a read, or a method RFC 9110 defines as idempotent), `guarded` (declares `idempotent`, and its runner actually sends the key), `unsafe`, `billed` (an AI call — paying twice is a budget decision, not a correctness one), `unknown` (a computed method), `opaque` (a callee it could not read). Two numbers carry it: **`retriedUnsafe`** is what the engine repeats *by itself*, needing no crash and no bad luck beyond a timeout — that is what `--strict` gates on — and **`recovery.verdict`** checks `recovery_policy: 'resume'`, which is an assertion about the graph made once in a dropdown and never verified since ([docs](../docs/REPEATS.md)) |
| `flowforge mutants <id> [--strict]` | **Would any of this workflow's checks notice if it were subtly wrong?** `test` says whether the suite passes; this says whether passing means anything. Introduces a plausible bug — a condition wired backwards, a threshold off by one, an approval gate deleted — and re-runs every check. A bug nothing catches is named precisely: not "coverage is 61%" but *"the approval gate can be deleted and every one of your tests still passes"*. Passes by default, because a survivor is evidence of a gap and not proof — an equivalent mutant cannot be killed by anything ([docs](../docs/MUTATION.md)) |
| `flowforge assertions <id> [--strict]` | **Things this workflow says must never happen**, checked against every run that does. `verify` proves properties of the *graph*; these check the ones no graph analysis reaches — data and outcomes. An assertion is a saved query: develop it with `flowforge query` against history, then pin the one you never want to match. Exits non-zero on a violation **and on a broken check** — one whose predicate throws on every run reports zero violations, and gating on violations alone would pass a build whose only check has never worked ([docs](../docs/ASSERTIONS.md)) |
| `flowforge query <id> "<fxl>" [--explain] [--json]` | **Ask a question of run history.** `insights`, `regressions` and `drift` each answer one *fixed* question; this answers the one you have at 3am. The predicate is **FXL** — the same language a condition node uses, so the whole stdlib is available and there is no second syntax: `status == "failed" and steps.charge.output.status >= 500`. Conjuncts that map onto execution columns are pushed into SQL, but every one is *also* evaluated by FXL, so a pushdown bug can cost speed and never change the answer. Exits **0** on matches, **1** on none, **2** on a bad predicate — so a query composes as a monitor, and a script can tell "no results" from "you typed it wrong" ([docs](../docs/QUERY.md)) |
