...
const { mergeGraphs, describeConflict } = require('../services/graphMerge')
...
// The common ancestor a three-way merge measures both sides against.
//
...
// (a version number or a snapshot id) overrides it for the case where the
// document was exported from an older release.
//
// No snapshots at all resolves to an empty base rather than an error. That
// reads every node as *added* by whichever side has it, which is the safest
// available interpretation: a merge with no ancestry can then never conclude
// that something was deleted, and deletion is the only outcome that loses work
// silently.
function resolveMergeBase(workflow, baseVersion) {
  ...
  if (!latest) {
    return {
      graph: { nodes: [], edges: [] },
      describe: { versionId: null, version: null, note: 'no version snapshots — merged against an empty base' },
    }
  }
  return {
    graph: parse(latest),
...
// ancestor can.
//
// The base is the workflow's **last deployed version snapshot** unless
// `baseVersion` names another, because a deploy is the point where the live
// workflow and the exported document provably agreed — the export was taken
// from one. A workflow with no snapshots merges against an empty base, which
// reads every node as added by whichever side has it: the safest possible
// interpretation, since nothing is deleted.
//
...
router.post('/workflows/:id/merge', tokenAuth('manage'), (req, res) => {
  try {
    ...
    if (!['manual', 'ours', 'theirs'].includes(strategy)) {
      return res.status(400).json({ error: 'strategy must be "manual", "ours", or "theirs"' })
    }

    const base = resolveMergeBase(workflow, req.body?.baseVersion)
    if (base.error) return res.status(400).json({ error: base.error })

    let ours = { nodes: [], edges: [] }
    try {
      const parsed = JSON.parse(workflow.graph_json)
      ours = {
        nodes: Array.isArray(parsed.nodes) ? parsed.nodes : [],
      ...
      /* unparseable stored graph merges as empty rather than failing */
    }

    const theirs = { nodes: graphData.nodes, edges: graphData.edges }
    const result = mergeGraphs(base.graph, ours, theirs, { strategy })

    // Lint the *result*, not the inputs. A merge of two individually valid
    // graphs can still produce one that won't run — a reference to a node the
    // other side deleted is the obvious case — and finding that out after
    // applying it would be the worst possible time.
    let lint = null
    if (result.graph) {
      const issues = lintGraph(result.graph, {
        secretNames: new Set(
          db.prepare('SELECT name FROM workspace_secrets WHERE workspace_id = ?')
            .all(workflow.workspace_id).map((r) => r.name)
        ),
    ...
    if (!req.body?.apply || !result.graph) return res.json(body)

    // Applying updates the canvas, not the deployment: a merged definition
    // becomes the live graph exactly as an edit would, and going live stays a
    // deliberate deploy. Audited, because a definition changed from outside the
    // app by a token is precisely what an audit trail is for.
    db.prepare('UPDATE workflows SET graph_json = ?, updated_at = ? WHERE id = ?')
      .run(JSON.stringify(result.graph), new Date().toISOString(), workflow.id)
    recordAudit(workflow.workspace_id, req.user.id, 'workflow.merged', {
      type: 'workflow',
      id: workflow.id,
      name: workflow.name,
      metadata: {
        baseVersion: base.describe?.version ?? null,
        strategy,
        ...result.summary,
      },
    })
    res.json({ ...body, applied: true })
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: 'Internal server error' })
  }
