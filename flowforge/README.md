...
Eight of the parts that were hard, each with a design record behind it. Every
one of them started as something the obvious implementation got wrong.
...
| **"Why didn't it send the email?"** | The question every workflow tool is asked and none of them answers. The run says `completed`, the email step says `skipped`, everything is green, and the customer did not get their receipt — so the run history hands back the fact the person asking already had. Answering it needs three things that were all in the same database and had never been joined up: **what the run did** (the rows the engine settles as `skipped`), **what gates what** (control dependence, which the effect report already computes), and **which way each gate went** (the decision's own recorded output). Together they turn a status into a sentence — *"High risk? was true, and that branch does not reach it: `total > 100`, total was 850"* — with the values read out of the recorded input rather than re-derived, because FXL is pure and the row is already there. It names the **deepest** gate that closed the path, since everything upstream of that is why *that* gate was reached; a field the input never had reports `not set` rather than falsy; and a `{{…}}` left/right comparison reports **no operands at all**, because that scope is not recorded per step and reconstructing it would be inventing a value and printing it as a fact. | [EXPLAIN.md](./docs/EXPLAIN.md) |
| **The diff nobody can read** | Four gates look at a candidate before it ships — `diff` says it drifted, `lint` says it is valid, `contract` says whether it breaks its callers, `preview` replays last week's traffic. None says what the edit does to the properties somebody was relying on, and the change that matters most is the one that is **structurally tiny**: delete one edge, wire a trigger at the node behind it, and an approval is no longer in front of a payment. Every node still lints, every type still checks, nothing is unreachable, and `preview` sees the same outputs because the payloads that reached the charge still reach it. The dominance analysis has known how to catch that since the day it was written — the only thing missing was running it **twice and subtracting**. Six analyses over both graphs, diffed. The discipline it lives on: a property already broken is **not a finding of this change**, the same rule mutation scoring uses when it credits a mutant only with what its mutation introduced. And the ungating leads *above* the broken guarantee, because a guarantee is already refused at deploy and an effect that quietly loses its gate is legal, deployable, and this is the only place it will ever be said. | [IMPACT.md](./docs/IMPACT.md) |
| **Everything fires at midnight** | Every timing analysis here is about *one* thing: the parallelism cap inside a run, the queue in front of a workflow, the longest chain in an execution. Nothing modelled the machine they all land on — and its load is **not random**, because cron is written by people and people write round numbers. Nobody schedules a report for 03:47. So: expand every cron over a week **in its own time zone** (two workflows both set to "midnight" in different zones do not collide, and expanding in UTC would invent one), treat each occurrence as occupying `[start, start + mean duration)` — because the 40-minute job that begins at midnight is still holding a worker when the 00:30 job lands — and sweep for max overlap. A workflow that has never run is **excluded and said so**, making the peak a floor rather than a figure built partly from a number nobody measured. The suggestion is one move, in minutes, inside the hour it already fires in: a 00:00 job at 00:17 still runs nightly, and at 01:00 it is a schedule its author did not ask for. | [SCHEDULE-LOAD.md](./docs/SCHEDULE-LOAD.md) |
| **A cap that governs a tenth of its traffic** | A cap is enforced by the *worker*, at pickup, and the worker only ever sees top-level runs. A **sub-workflow call executes inside the caller's engine loop, holding the caller's slot** — it never asks for one of the callee's, so it never queues and the callee's cap never sees it. The capacity report was counting those runs anyway, and the error ran in two directions at once: they inflated the arrival rate it predicts a wait from, and — since a called run's `started_at` is effectively its `created_at` — they filled the observed-wait sample with zeros. Those are the *exact two numbers* the calibration block compares, so a mostly-called workflow could report **`agrees`** on traffic neither described, which is the worst available failure for a report whose whole argument is "this model has been checked". Now excluded, with `governance.share` saying what the rest of the payload describes, and a distinct `not-governed` verdict — because "wait for more history" is the wrong instruction when the history is already arriving and simply bypassing the number. | [CAPACITY.md](./docs/CAPACITY.md#traffic-the-cap-does-not-govern) |
...
<details>
<summary><strong>The other twenty-seven</strong> — the rest of the hard parts, same format</summary>
...
| **One box, an entire workflow** | The effect report answers *what can this do to the outside world* over one graph, completely — and a **sub-workflow node breaks that**. On the canvas it's one box; at run time it's another whole workflow with its own HTTP calls and its own gates, and the honest per-graph answer is *"calls workflow 4f2a"*, which tells a reviewer nothing. So the call is expanded into what the callee actually does. The part that has to be right is the preconditions, and it's a **conjunction**: the caller's gate on the call *and* the callee's gate on the effect. Keeping only one half is wrong in a different direction each way — and an effect that is unconditional *inside* the callee must not report as `always` once it's reached through a gate. | [EFFECTS.md](./docs/EFFECTS.md#across-the-sub-workflow-boundary) |
| **Where do I even start?** | Every analysis here answers a question about **one** workflow — and each one assumes the hardest part is already done, that somebody knew which workflow to open. Nobody has one workflow. So: rank the workspace by **outward actions per day** — the effects a run can reach (including the ones several sub-workflow calls away) times how often it actually runs. Three decisions carry it. It reports an **interval**, because most effects are gated and nothing here evaluates a gate; the ranking is by the *ceiling*, since the report exists to find workflows nobody has checked and an untested gate is not evidence. The rate counts only runs **nobody else started** — a sub-workflow call writes its own execution row, and counting those would bill the same charge twice and rank the subroutine above the workflow that decides to invoke it. And assurance is **counted, never scored**: four scenarios don't make a workflow four units safer, so the four kinds of check sit beside the number unweighted and the queue is the plainest filter over them — consequence, and nothing watching it. | [EXPOSURE.md](./docs/EXPOSURE.md) |
| **What happens twice** | Every static check here asks whether *one run* is right. Three mechanisms make a step run a second time, and the uncomfortable one is **node retries**: three attempts by default, on every run, needing no crash and no bad luck beyond a slow response — and a retry fires on a **timeout**, exactly the case where the far side may already have done the work. So each step gets a verdict, and the boundaries are the design. `PUT`/`DELETE` are safe, which is a claim about *RFC 9110* rather than about the server — flagging every PUT would bury the POST that is one. An AI call is `billed`, not `unsafe`, because paying twice is a budget decision and folding them together makes every AI workflow look broken. A computed method is `unknown` rather than unsafe, because a report that flags everything is a report somebody turns off. And the half that makes it an analysis: **`recovery_policy: 'resume'` is an assertion** — *"for a graph whose steps are idempotent, which only its author can know"* — made once, in a dropdown, about a graph edited fifty times since. This is the only place it is checked, including against a charge three boxes and one call away. | [REPEATS.md](./docs/REPEATS.md) |
| **Testing the tests** | Every check here answers *does this workflow pass?* — the linter, the type checker, test scenarios, guarantees. None answers the one underneath: **if it were subtly wrong, would any of them notice?** A suite of three scenarios all asserting `status == "completed"` passes on a workflow with its approval gate deleted. So: **mutation testing**. Introduce a plausible bug — a condition wired backwards, a threshold off by one, a gate removed — and re-run every check. The report names what got through: not "coverage is 61%" but *"the approval gate can be deleted and every one of your tests still passes"*. Mutants are credited only with what their *mutation* broke (compared against a baseline, so a graph that doesn't lint can't score 100 on inherited errors), and it **passes by default** — an equivalent mutant can't be killed by anything and identifying those is undecidable. Each survivor also comes with **the input that would have caught it**: for an off-by-one, the solver is asked for an input satisfying `(orig) and not (mutant)` — the boundary value, not a passing one that both graphs agree about. | [MUTATION.md](./docs/MUTATION.md) |
| **Assertions over production** | Guarantees prove properties of the **graph** — *can this ever charge without the approval?* — statically, by dominance. They can only see what the graph's shape decides, and the things that break production don't: *a run must never complete with the charge step returning 4xx*. So pin a **saved query that must never match**, developed with `flowforge query` against history and then kept as the same string. Checked on the engine's terminal hook rather than a sweep — a sweep needs a watermark, and a watermark can skip a run (missed forever) or replay one (alerts twice); the run in front of you is exact. The design's real point: an assertion whose predicate *throws* on every run reports zero violations, so evaluations that complete are counted apart from ones that throw, and **broken is never folded into holding**. | [ASSERTIONS.md](./docs/ASSERTIONS.md) |
| **Querying run history** | Every report here answers one *fixed* question well — durations, regressions, drift, capacity. None answers the one somebody has at 3am, because that one is always specific: *which runs last week failed at the charge step with a 5xx, for orders over a thousand?* Today that's a SQL prompt and a `json_extract` incantation, so it's a question only somebody with database access can ask. **FXL becomes the query language** — the same one condition nodes use, so there's no second syntax and the whole stdlib works. Conjuncts that map onto columns are pushed into SQL, but **every conjunct is also evaluated by FXL**, so a pushdown bug can cost speed and never change the answer — which matters, because FXL string-compares (`undefined >= 400` is *true*) where SQL's three-valued logic drops the row. | [QUERY.md](./docs/QUERY.md) |
...
client files (737), 32 CLI files (425 tests), and 120 pytest tests for the AI
